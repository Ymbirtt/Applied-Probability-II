...
\begin{enumerate}
\item
A continuous time process with stationary, independent increments,
such that for $t>0$, $h>0$, $n \in \mathbb{N}^{+}$
\begin{align*}
\pr (N(t+h) < n | N(t) = n) &= 0\\
\pr (N(t+h) = n | N(t) = n) &= 1-\lambda(h) + o(h)\\
\pr (N(t+h) = n+1 | N(t) = n) &= \lambda(h) + o(h)\\
\pr (N(t+h) > n+1 | N(t) = n) &= o(h)
\end{align*}
\item
\begin{align*}
...
poisson.
\item
Following a really useful email conversation with Carl Martyres, I've been able to
make a little progress on this one. I'm still not completely comfortable with it, 
but here goes.

$\pr (D(t) = k+m | N(t) = k) = \pr(D(t)-N(t)=m | N(t)=k)$ is the probability
that exactly $m$ decays go undetected given we have detected $k$ decays by time t.
Since the number of decays by time $t$ follows a $Po(\mu pt)$ distribution, by 
reversing the process we also have that the number of decays not detected follows
a $Po((1-p)\mu t)$ distribution, so 
$$
\pr (D(t)-N(t) = m | N(t) = k) = \frac{e^{-(1-p)\mu t}((1-p)\mu t)^m}{m!}
$$

The resulting equation, however, does not depend on $k$, so by partitioning on $k$ 
we have that
\begin{align*}
\pr (D(t)-N(t) = m)&= \sum_{k=0}^\infty \pr(D(t)-N(t)=m | N(t) = k)\pr(N(t) = k)\\
&= \frac{e^{-(1-p)\mu t}((1-p)\mu t)^m}{m!}\sum_{k=0}^\infty \pr(N(t) = k)\\
&= \frac{e^{-(1-p)\mu t}((1-p)\mu t)^m}{m!}\\
&= \pr (D(t)-N(t) = m | N(t) = k)
\end{align*}

So $\pr (D(t)-N(t) = m)=\pr (D(t)-N(t) = m | N(t) = k)$, so $D(t)-N(t)$ and $N(t)$
are independent.
\end{enumerate}
\end{enumerate}
\clearpage
...
If the process starts by moving upwards, so $Y_1 = 1$, we have a process similar 
to before, with $a=0,b=K, X_0 =1$, so
\begin{align*}
s_1 = r_1 &= \frac{b-1}{a+b}\\
&= \frac{K-1}{K}
\end{align*}
Similarly, if the process starts by moving downwards, $Y_1 = -1$, we have
...
||\underline{\mu}||_1 = 1$
\begin{align*}
\iff \mu_i &= \sum^n_{j=1}q_{ji}\mu_j\\
&= \sum^n_{j=1}\delta p_{ji}\mu_j + (1-\delta)\mu_i \quad \mbox{ since
$p_{ii}=0$}\\
\iff \delta \mu_i &= \sum^n_{j=1}\delta p_{ji} \mu_j\\
\iff \mu_i &= \sum^n_{j=1}p_{ji}\mu_j
\end{align*}
